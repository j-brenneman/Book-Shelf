...
var bookInsert = function (req) {
  ...
  return Library.create(
    ...
    if(req.body.quotes) {
      ...
      return Promise.all(quotes).then(function (quote) {
        return Quotes.create({userID: req.params.id, bookID: book.id, quote: quote})
      })
    }
  })
}
...
var findBooks = function (req) {
  return Library.find({userID: req.params.id}).then(function (books) {
    return Promise.all(books.map(function (book) {
      return Quotes.find({bookID: book._id}).then(function (quotes) {
        book.quotes = quotes;
        return book;
      })
    }))
...
var findBook = function (req) {
  return Library.find({_id: req.params.id}).then(function (book) {
    return Quotes.find({bookID: book[0]._id}).then(function (quotes) {
      book.quotes = quotes;
      return book;
    })
  })
